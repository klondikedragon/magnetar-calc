...
- The input palette uses a 52px decorative Magnetar icon at left, rendered at 40% opacity; the memory controls and input-mode dropdown remain right-aligned, with the dropdown below the memory controls. Preserve the semantic `Input palette` label for assistive technology without restoring the visible heading.
- Appearance is a device-local System/Light/Dark preference and is never included in notebook or view exports. Build both themes from semantic CSS color tokens.
- Scientific is the single general-purpose palette; migrate legacy Calculator and Trigonometry selections to it. Keep one emphasized equals key at bottom-right and reserve the unused top-right slot for a future action.
- Palette equals saves and then wraps the whole active expression for operator-based continuation; Enter saves without rewriting user-authored syntax. Copy actions confirm success inside their initiating button rather than through displaced success text.
