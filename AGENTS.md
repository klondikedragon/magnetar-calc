...
- The input palette uses a compact 40px decorative Magnetar icon at left; the memory controls and input-mode dropdown remain right-aligned, with the dropdown below the memory controls. Preserve the semantic `Input palette` label for assistive technology without restoring the visible heading.
